...
describe('CommentRepositoryPostgres', () => {
  ...
  describe('isCommentExist function', () => {
    ...
    it('should return false if comment not exists', async () => {
      ...
      // Action & Assert
      await expect(commentRepositoryPostgres.isCommentExist('comment-123')).rejects.toThrowError(NotFoundError);
    });
  });
  ...
  describe('isCommentOwner function', () => {
    it('should return true if user is comment owner', async () => {
      ...
      // Action & Assert
      await expect(commentRepositoryPostgres.isCommentOwner('comment-123', 'user-123')).resolves.not.toThrowError(AuthorizationError);
    });
    ...
    it('should return false if user is not comment owner', async () => {
      ...
      // Action & Assert
      await expect(commentRepositoryPostgres.isCommentOwner('comment-123', 'user-456')).rejects.toThrowError(AuthorizationError);
    });
  });
